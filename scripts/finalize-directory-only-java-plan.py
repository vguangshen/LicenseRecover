...
# Remove catch-all data completely from the plan.
start='    // Must stay byte-for-byte equivalent in meaning to LicenseRecover.ALL_NUMS.\n'
end='    public final boolean detected;\n'
a=p.find(start)
if a<0: raise SystemExit('fallback constant start missing')
b=p.find(end,a)
if b<0: raise SystemExit('fallback constant end missing')
p=p[:a]+p[b:]
...
'''
new='''        String dataSoft = readElement(dataConfig, "SoftVersionID");
        String dataRegInfo = normalizeCsv(readElement(dataConfig, "regInfo"));
        String classesRegInfo = normalizeCsv(readElement(classesConfig, "regInfo"));
        ...
        // an identity only from target-directory evidence: parsed Global/RegisterUtil,
        ...
        boolean directoryRegStr = directoryMapping != null
                || dataRegInfo != null || classesRegInfo != null || recoveredLocalRegStr != null;
'''
p=replace_once(p,old,new,'strict identity block')

old2='''    private static String resolveRegStr(File root, String softId, String runtimeProduct, File lib) {
...
'''
new2='''    private static String resolveRegStr(File root, String runtimeProduct, File lib) {
        String data = normalizeCsv(readElement(new File(root, "data" + File.separator + "config.xml"), "regInfo"));
        if (data != null) return data;
        String classes = normalizeCsv(readElement(new File(root, "WEB-INF" + File.separator
                + "classes" + File.separator + "config.xml"), "regInfo"));
...
'''
p=replace_once(p,old2,new2,'remove RegStr fallbacks')
PLAN.write_text(p,encoding='utf-8',newline='\n')

# Update smoke tests so historical maps are diagnostics only, never executable proof.
...
for old,new in replacements.items():
    t=replace_once(t,old,new,'test replacement '+old.splitlines()[0][:60])
...
if 'Java plan still contains catch-all fallback RegStr' not in v:
    if anchor not in v: raise SystemExit('verify anchor missing')
