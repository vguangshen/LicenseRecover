...
v1.2.44 起，备份清理由单独的“备份整理”栏合并进“操作”单选项，与“一键恢复授权”和“方式三”并列为“批量删除备份”。保留份数仍在同一“选项”行配置，选择该操作后普通写入/联网/预览选项自动禁用；执行时无需先扫描应用，直接对父目录下 LicenseRecover 自身生成的 `prewrite` / `prepatch` / `preoneclick` 和已知旧版时间戳备份进行安全扫描与确认删除。
