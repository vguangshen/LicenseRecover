...
# Idempotency: the workflow fires again after it commits the refactor.
auto_path = "src/main/java/LicenseRecoverModernGUIAutoRecovery.java"
auto = rd(auto_path)
if "[java-chain] target-native isolated helper; product=" in auto:
    print("Java target-native staged recovery is already wired")
    raise SystemExit(0)

# Tighten helper isolation: target code may see the JRE/platform loader, never the
# LicenseRecover system classpath containing bundled compatibility libraries.
host_path = "src/main/java/LicenseRecoverJavaHost.java"
host = rd(host_path)
host = one(
    host,
...
end = auto.index("    private static Result recoverDotNet(", start)
replacement = r'''    private static Result recoverJava(Detection d, boolean backup, boolean blockNet, boolean dryRun, Consumer<String> log) throws Exception {
...
'''
auto = auto[:start] + replacement + auto[end:]
wr(auto_path, auto)

# Smoke tests: helper JVM classpath must not contain target jars and constructor
# generations remain compatible.
test_path = "src/test/java/RefactorSmokeTest.java"
test = rd(test_path)
old_test = '''        String recoveryCp = LicenseRecoverModernGUIAutoRecovery.buildJavaRecoveryClasspath(
...
new_test = '''        String recoveryCp = LicenseRecoverModernGUIAutoRecovery.buildJavaRecoveryClasspath(
                toolCpDir.toFile(), targetRuntime.toFile());
...
test = one(test, old_test, new_test, "Java classpath smoke test")
wr(test_path, test)

# Overlay/package guards: ship the helper and enforce its architecture in CI.
verify_path = "scripts/verify.ps1"
verify = rd(verify_path)
verify = one(
    verify,
...
anchor = "$autoSource = Get-Content -LiteralPath (Join-Path $mainSourceDir 'LicenseRecoverModernGUIAutoRecovery.java') -Raw\n"
verify = one(
    verify,
    anchor,
    anchor + "$javaHostSource = Get-Content -LiteralPath (Join-Path $mainSourceDir 'LicenseRecoverJavaHost.java') -Raw\n",
    "java host source variable",
)
guard = """if (-not $javaHostSource.Contains('class ChildFirstLoader') -or -not $javaHostSource.Contains('checkConnect(String host, int port)')) { throw 'Java target-native isolated host/network guard is missing.' }
...
"""
marker = "Write-Host 'Building deterministic runtime overlay...'\n"
verify = one(verify, marker, guard + "\n" + marker, "java native host source guards")
wr(verify_path, verify)

print("Java target-native staged recovery wired")
